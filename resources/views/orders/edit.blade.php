...
@section('content')
    ...
    <form action="{{ route('orders.update', $order->id) }}" method="POST" id="orderForm">
        ...
        <div class="form-group">
            ...
            <select name="drink_id" id="drink_id" class="form-control" required>
                @foreach (\App\Models\Drink::all() as $drink)
                    <option value="{{ $drink->id }}" data-price="{{ $drink->price }}" {{ $order->drink_id == $drink->id ? 'selected' : '' }}>
                        {{ $drink->name }} ({{ number_format($drink->price, 2) }} VND)
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" step="0.01" name="total_price" id="total_price" class="form-control" min="0" value="{{ $order->total_price }}" readonly>
        </div>
